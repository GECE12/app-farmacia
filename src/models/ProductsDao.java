    ...
    // eliminar productos
     public boolean deleteProductQuery(int id){
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "No puedes eliminar el producto que tiene relacion con otra tabla");
            return false;
        }
     }
     ...
     String query = "SELECT pro. * ca.name AS category_name FROM products pro "
             + "INEER JOIN categories ca ON pro.category_id = ca.id WHERE pro.id = ?";
     Products product = new Products();
